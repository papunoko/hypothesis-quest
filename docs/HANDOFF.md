...
## 最終ラウンドの試遊・修正（2026-09-21・コミット/公開準備）
...
- 前ラウンドのF-11〜16（支援/核心度/スクロール/提出後の再開/新セッション）も今回まとめてコミット対象。未知の未追跡 `x` は対象外。
...
- 公開先は既存の `ebiharadev.org`、期限は2026-10-01 00:00 JSTのまま。Route・D1・Secrets・停止中のサブドメインには変更なし。公開完了後にバージョンと確認結果をここへ追記する。
