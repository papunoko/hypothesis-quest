...
## 最終ラウンドの試遊・修正（2026-09-21・コミット/公開済み）

- **公開済み**: 実装 `56df141`、Worker `fd3a9aff-4c7a-47e5-9ac6-46fad55f0cb4`、2026-09-21 01:08 JSTに100%配信。https://ebiharadev.org 。本番7/7（実Jev/LLM5＋セッションUI2）通過、内ブラウザでも実回答を目視。詳細は [deployment.md](deployment.md)。

- 最終依頼は「試遊と修正してコミット＆デプロイ」。F-07（確認済みH1が未確認一覧に残る）を修正。予想・既読はクライアントのみ、DB migrationなし。
- F-10は一部対策: 内部フィールド名5種を含む生成文は定型へ。7事例の一致と未測定の変更後性能を分け、提示コードに根拠があるのに「未確認」と返す矛盾を検問。過去の保存済み文章は変更しない。
- 通常回答が「引数1つ/2つ」を漏らして検問を通る実例も発見。通常回答を事例IDと実結果へ限定し、比較の着眼点を名指す文はspoilerと明記。閾値・問い合わせ回数は変更しない。固定検問10＋生成4、計14/14。結果と失敗例は [最終試遊記録](eval/journey-check-2026-09-21-final-round.md)。検問の一般保証ではない。
- 本文を見た再試遊: 127.0.0.1の隔離Cookieで質問→レビュー→追加事例→質問再開。修正後の実LLM回答は照合と未測定性能を分離、H1が未確認一覧から外れる。既知の正解を使った機械試遊で、人の理解・転移の証明ではない。
- 前ラウンドのF-11〜16（支援/核心度/スクロール/提出後の再開/新セッション）も今回まとめてコミットした。未知の未追跡 `x` は対象外・未変更。pushは未実施。
- 検証: 単体40件、TypeScript、レビュー回帰5件、Workerビルドと30件（UI25＋実API5）通過。最後の通常回答補強後も再ビルド、単体40件と実API5件（49.4秒）を再実行して通過。jev-lintは対象4ファイル41 subjects/0 findings（31 cached）、最終narration追加検査8 subjects/0 findings（5 cached）。全体cleanの主張ではない。
- 公開先は既存の `ebiharadev.org`、期限は2026-10-01 00:00 JSTのまま。Route・D1・Secrets・停止中のサブドメインには変更なし。本番migrationなし。3002（Next dev）と8787（公開と同じソースのWorker）を再起動済み。コミット対象37ファイルと成果物1219ファイルに既知の秘密値がないことを検査（一般トークンパターンの4件はNextの識別子を誤検出、個別確認済み）。

## ブラウザ試遊→修正→再試遊（2026-09-21・未デプロイ・未コミット）

以下は公開前の作業記録。現在のコミット/公開状態は冒頭の最終ラウンドを優先する。

- 9/20の視覚的試遊で見つかった4点を修正。送信済みの同じ入力には自動の助け舟を出さない（明示的な再送・手動ヒントは可能）。編集・IME・事例移動・提出・終了・閉じる操作で支援リクエストを中断し、世代番号でも古い応答を捨てる。質問用の送信ボタン・説明文に追従。
- 核心度は `max(types, singleFast)` から、表示専用の `coreRelevance` Noulへ分離。質問・肯定・否定いずれでも「型／個数の話題に触れるか」を読む。正誤・7事例の照合・クリア条件には不使用。未読・旧データは「—」、空欄は0.0。意味を常時表示する。既存の1リクエストに1問追加（問い合わせ回数は増えないがトークンは増える）。
- ヒントの「書き方まで同じなら記憶を返す」は「書き方は関係ある？」へ。最初の仮説＋書き方/型/順番の質問を用意。判定閾値を下げて候補を通す変更はしていない。
- 開始・結果・レビュー・再開時に結果見出しへ移動し、PCの入力列内スクロールも戻す。1回目の修正はスマホの `display: contents` な外枠を対象にして失敗。実体のある見出しへ変更して回帰を通した。
- 追加依頼: レビュー画面の「質問・観察を続ける」で、帳面・確認済み事例・レビュー下書き/判断・提出回答を残して探索へ戻れる。レビューを一度開いた後は非表示で保持し、戻るたびに初期化しない（再読込では未提出の下書きは失われる）。
- ヘッダーの「新しいセッションを始める」は確認後に `POST /api/session` でアプリのHttpOnly Cookieのみ更新して再読込。旧履歴へUIから戻れなくなること、DBは削除しないこと、同一ブラウザの他タブにもCookieが共有されることを告知する。同一OriginのPOST限定、Workerでは既存POST制限・公開期限が適用される。DB migrationなし。
- 表示専用の核心度だけが欠落/不正なら「—」にし、比較の読み取りは続ける。6比較軸・種別/話題の異常は従来どおり拒否する。
- 試遊: Codex内ブラウザで3002の画面を見ながら、候補挿入→書き方の回答→型の回答→途中レビュー→再質問→下書きと提出回答の維持まで操作。前回のCookieを継続しており初見・別セッションの評価ではない。実Jevの候補/核心度16件通過。最初の核心度案12/16の失敗記録も保存。実LLMテストの旧ラベル参照を修正。
- 最終検証: 単体39/39、TypeScript、`git diff --check`、Workerビルド、ローカルWorkerのPlaywright **30/30**（UI25＋実Jev/LLM5）通過。対象6ファイルのjev-lintは26 subjects/0 findings（全体cleanの意味ではない）。3002（Next dev）と8787（最新ローカルWorker）を起動中。Cookie切替後の空の帳面/提出履歴・旧セッションのDB保持も隔離ブラウザで確認。本番へのデプロイ・コミットなし。
- 記録: [eval/journey-check-2026-09-21-support-ux.md](eval/journey-check-2026-09-21-support-ux.md)。本番の期限・Route・DB・Secretsは変更しない。未知の未追跡ファイル `x` は触っていない。

## 入力欄まわりの作り直し（2026-09-20 深夜・ローカル検証済み・未デプロイ・未コミット）

この節は実装当時の記録。現在は `3f714d7` でコミット済みで、9/20のブラウザ試遊時には8787もこの版だった。核心度・助け舟の最新仕様は上の9/21節を優先する。
