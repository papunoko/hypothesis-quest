...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entry = store.list(session).find((e) => e.id === body.entryId);
    ...
    store.saveHint(session, entry.id, hint);
    return Response.json({ narration: hint, entries: store.list(session) });
