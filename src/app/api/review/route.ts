...
export async function GET() {
  try { return Response.json({ submissions: await notebook().submissions(await notebookSession()) }, { headers: { "Cache-Control": "no-store" } }); }
...
export async function POST(request: Request) {
  ...
  try {
    const store = notebook(); const session = await notebookSession(); const entries = await store.list(session);
    const previous = (await store.submissions(session)).find((s) => s.id === id);
    if (previous) {
    ...
    const submission: Submission = await store.submit(session, { ...draft, narration });
    // Another request may have won the insert while the LLM was running.
    if (submission.hypothesis !== hypothesis || submission.review !== review || submission.decision !== body.decision) return Response.json({ error: "別の内容で同じ提出IDは使えません。再提出してください。" }, { status: 409 });
