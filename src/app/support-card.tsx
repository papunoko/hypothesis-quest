...
import type { InputKind } from "@/lib/lru-input";
...
export function SupportCard({ kind, source, busy, sendLabel, inputKind, onInsert, onNext, onOpenMap, onSend, onClose }: {
  ...
  return <aside className="support-card" role="status" aria-label="助け舟" data-kind={kind}>
    ...
    <p>{kind === "send" && inputKind === "question" ? "質問として読み取れています。送信すると、用意した比較テストで確かめられるか調べます。" : COPY[kind].body}</p>
    ...
    {kind === "send" && <div className="row"><button disabled={busy} onClick={onSend}>{sendLabel}</button></div>}
