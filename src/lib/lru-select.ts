...
export function predictLru(c: LruCase, r: LruReading): { prediction: LruPrediction; confidence: number } {
  ...
  for (const axis of axes) {
    const possible = r[axis] >= 0.6 ? [true] : r[axis] < 0.4 ? [false] : [false, true];
  ...
  const relevant = axes.filter((axis) => readings.some((reading) => apply({ ...reading, [axis]: !reading[axis] }) !== prediction));
