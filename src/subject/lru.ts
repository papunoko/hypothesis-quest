...
export const LRU_CASES: LruCase[] = [
  ...
  { id: "L2", title: "省略していた0を書く", situation: "yの初期値は0。省略しても、0を書いても、関数に渡るxとyの値は同じです。", calls: ["f(1)", "f(1, 0)"], typed: false, features: features({ form: false }), actual: "computed", observation: "本体が合計2回動きました。y=0を省略した呼び出しと、明示した呼び出しは別の記憶です。", companion: "計算に使う値は同じ。それでも呼び出し方が違うと、もう一度計算しました。", evidence: evidence(33774, "key = args", 466) },
  ...
  { id: "L4", title: "名前付き引数の順番を入れ替える", situation: "xは5、yは6のまま。書く順番だけを入れ替えます。", calls: ["f(x=5, y=6)", "f(y=6, x=5)"], typed: false, features: features({ order: false }), actual: "computed", observation: "本体が合計2回動きました。キーワード引数の順番も記録に残ります。", companion: "同じ名前、同じ値でも、順番が変わると別扱い。PRの追記が挙げていたのは、このような例です。", evidence: evidence(29203, "for item in kwds.items():\n    key += item", 469) },
  ...
  { id: "L7", title: "typed=Trueを指定する", situation: "L6と同じ2つの呼び出しを、型も区別する設定typed=Trueで試します。", calls: ["f(1, 2)", "f(1.0, 2)"], typed: true, features: features({ types: false, typed: true }), actual: "computed", observation: "本体が合計2回動きました。typed=Trueでは、引数の型も記録に含まれます。", companion: "設定を変えると、2個の引数でも型を区別します。この設定まで説明に含められそうですか。", evidence: evidence(13227, "if typed:\n    key += tuple(type(v) for v in args)", 471) },
];
...
export const LRU_AXES = {
  ...
  types: { label: "型が違えば常に別の呼び出し", instructions: "Does `hypothesis` explicitly name argument type as a matching requirement?", criteria: { true: "型まで同じなら, 型が違えば別, 1と1.0は別. Matching types is a condition for reusing results.", false: "Types are absent or ignored. A rule ONLY about typed=True or a single-argument special case is not a general matching requirement." } },
